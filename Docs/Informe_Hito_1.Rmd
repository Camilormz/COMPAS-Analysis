...
```
De la misma forma se hace un análisis por sexo de los puntajes.
```{r}
...
```
En el gráfico mostrado es notable la diferencia de proporción entre las razas a medida que aumentan los puntajes, en el caso de la gente hispana, se presenta una leve baja a medida que aumenta el puntaje, el principal contraste lo presenta entre las personas caucásicas y afro-americanas, donde se puede ver con claridad que el algoritmo les asignó en general un puntaje significativamente mayor que a las personas caucásicas, las que dominan los puntajes menores.
...
Se observa que dada la exploración de datos realizada en el hito 1 se pudieron definir correlaciones entre las variables de entrada y además se pudo establecer dependencias entre ellas. Como trabajo propuesto para el hito 2 sería encontrar otra forma de medir el sesgo dentro del dataset, por otro lado hacer un clasificador que sea capaz de predecir los campos de entrada a partir de la salida y finalmente predecir variables ocultas.

El objetivo del hito 1 es mediante una exploración de datos entender y trabajar con los datos para buscar posibles hipótesis a preguntas que se iran respondiendo en los siguientes hitos del proyecto.

En este trabajo tuvo algunos incovenientes como la salida de dos alummnos del proyecto lo que dificulto la distribución del trabajo y la carga de trabajo que cada integrante realizó.

La distribución de trabajo fue la siguiente:
Ignacia Aguilera: redactó la problematica central y la conclusión. Además realizó parte de la exploración de datos realizando los graficos presentados.
Camilo Ramírez: redactó la introducción, descripción de los datos y se encargo de la limpieza de la exploración de datos.
Sebastián Zapata: se encargó de realizar la exploración de datos encontrando correlaciones y realizó el diagrama Entidad - Relación
